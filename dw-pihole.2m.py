...
base_url = "http://192.168.1.xx/admin"
...
url_summary = "%s/api.php?summary&auth=%s" % (base_url, api_token)
...
def bitbar_online():
    ...
    # imos buscar se hai updates e engadir "Update" a este string
    updateAvailable = False 
    if versions['FTL_update'] or versions['core_update'] or versions['web_update']:
    ...
    if updateAvailable:
    	print("Update Available | color=red templateImage=%s" % globals()['icon_%s' % icon_type])
    else:
